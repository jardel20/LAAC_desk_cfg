...
def apply_base_config(options):
    ...
    # CORRIGIDO: Toolbox com saveAsImage funcionando
    options["toolbox"] = {
        "feature": {
            "dataView": {
                "show": True,
    ...
    # CONFIGURAÇÃO CRÍTICA: Garantir que o renderer seja canvas
    if "renderer" not in options:
        options["renderer"] = "canvas"  # Usar canvas em vez de SVG

    # Configurações de exportação
    options["useUTC"] = True
    options["animation"] = False  # Desativar animação para exportação
...
with st.sidebar:
    ...
    with st.expander("📄 Arquivos", expanded=False):
        ...
        with col1:
            # Botão para gerar arquivo individual com curva completa
            if st.button("⚡ Curva", use_container_width=True,
                         help="Gera arquivo com curva gaussiana completa (múltiplos pontos)"):
        ...
        with col2:
            # Botão para gerar arquivo simplificado com ICE
            if st.button("📊 Linear", use_container_width=True,
                         help="Gera arquivo com apenas início e fim com ICE (2 linhas)"):
                # Obter dados do canal
                dados = sistema.get_dados_canal(canal_nome)
                params_temp = st.session_state.parametros_temporais

                # Criar conteúdo simplificado com ICE
                conteudo_arquivo = sistema.gerar_conteudo_lamp_ice(
                    dados, params_temp)

                # Nome do arquivo baseado na seleção (adiciona _ICE)
                nome_arquivo = arquivo_selecionado.replace('.txt', '_ICE.txt')
        ...
        with col3:
            ...
            if st.button("📦 Todos", use_container_width=True,
                ...
                with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                    ...
                    # Primeiro: arquivos com curva completa
                    ...
                    # Segundo: arquivos com ICE simplificado
                    for nome_arquivo, canal in arquivos_para_gerar:
                        dados = sistema.get_dados_canal(canal)
                        params_temp = st.session_state.parametros_temporais
                    ...
                    ESTRUTURA DO ZIP:
                    ├── curva_completa/        - Arquivos com curva gaussiana completa
                    ...
                    │
                    └── ice_simplificado/     - Arquivos simplificados com ICE
                    ...
                    HH_INICIO 00 00 ICE
                    HH_FIM 00 00 ICE
                    (Apenas 2 linhas: início e fim com valor de ICE)
                    """
                    zip_file.writestr("README.txt", readme_content)

                    # Adicionar também um arquivo CSV com os ICEs
                    ice_data = []
        ...
        # Área para previews (ocupará largura total abaixo dos botões)
        # Preview do arquivo Curva (se gerado)
        if st.session_state.get(f'download_{arquivo_selecionado}', False):
            with st.expander(f"👁️ Pré-visualizar {arquivo_selecionado} (Curva)", expanded=False):
                dados = sistema.get_dados_canal(canal_nome)
                params_temp = st.session_state.parametros_temporais
                conteudo_arquivo = sistema.gerar_conteudo_lamp(
                    dados, params_temp)
        ...
        # Preview do arquivo Linear (se gerado)
        if st.session_state.get(f'download_ice_{arquivo_selecionado.replace('.txt', '')}', False):
            with st.expander(f"👁️ Pré-visualizar {arquivo_selecionado.replace('.txt', '_ICE.txt')} (Linear)", expanded=False):
                dados = sistema.get_dados_canal(canal_nome)
                params_temp = st.session_state.parametros_temporais
                conteudo_arquivo = sistema.gerar_conteudo_lamp_ice(
                    dados, params_temp)
                st.code(conteudo_arquivo, language="text")
                st.info(f"ICE do canal: {dados['ICE']:.1f} μmol/m²/s")

        # Resumo dos ICEs (se gerado Todos)
        if st.session_state.get('download_all_formats_zip', False):
            with st.expander("📊 Valores de ICE utilizados", expanded=False):
                for canal_nome, canal_display in [('vermelho', 'Vermelho'), ('azul', 'Azul'), ('branco', 'Branco')]:
                    dados_canal = sistema.get_dados_canal(canal_nome)
                    st.metric(
                        f"ICE {canal_display}",
                        f"{dados_canal['ICE']:.1f} μmol/m²/s",
                        f"DLI: {dados_canal['DLI_final']:.3f} mol/m²")
...
def exibir_configurar_canais():
    ...
    **NÃO** é de banda espectral.
    ...
    with col2:
        ...
        # Linha 1: Rótulos com cores
        col_label1, col_label2, col_label3 = st.columns(3)
        with col_label1:
            st.markdown('<div style="text-align:left;"><span font-weight:bold;">Azul</span></div>',
                        unsafe_allow_html=True)
        with col_label2:
            st.markdown('<div style="text-align:left;"><span font-weight:bold;">Vermelho</span></div>',
                        unsafe_allow_html=True)
        with col_label3:
            st.markdown('<div style="text-align:left;"><span font-weight:bold;">Branco</span></div>',
                        unsafe_allow_html=True)
